...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    user: process.env.ALERT_EMAIL, // Your Outlook email
    pass: process.env.ALERT_PASSWORD, // App password for Outlook
  },
  tls: {
    ciphers: "SSLv3",
  },
...
const sendAlertEmail = (ip) => {
  const mailOptions = {
    from: process.env.ALERT_EMAIL,
    to: process.env.ALERT_EMAIL, // Send the alert to yourself
    subject: "🚨 API Abuse Detected!",
    text: `An IP (${ip}) has exceeded the rate limit on /generate-doc.`,
  };
